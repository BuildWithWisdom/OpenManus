...
import { Copy, Check, Maximize2, Loader2, X, ZoomIn, ZoomOut, RotateCcw } from 'lucide-react';
...
export const FormattedResponse: React.FC<FormattedResponseProps> = React.memo(({ content, onExpandMermaid }) => {
  const [internalExpandedSvg, setInternalExpandedSvg] = useState<string | null>(null);
  const [diagramZoom, setDiagramZoom] = useState<number>(1);
  const [isDragging, setIsDragging] = useState<boolean>(false);

  const zoomWrapperRef = React.useRef<HTMLDivElement>(null);
  const panPosRef = React.useRef<{ x: number; y: number }>({ x: 0, y: 0 });
  ...
  const cleanContent = useMemo(() => {
    if (!content) return '';
    return content.replace(/^(#{1,6})\s*#+\s*/gm, '$1 ');
  }, [content]);

  const cleanHeadingChildren = (children: any): any => {
    if (typeof children === 'string') {
      return children.replace(/^#+\s*/, '');
    }
    if (Array.isArray(children)) {
      return children.map((c) => (typeof c === 'string' ? c.replace(/^#+\s*/, '') : c));
    }
    return children;
  };
  ...
  return (
    <>
      <ReactMarkdown
        ...
        components={{
          h1({ children }) {
            const textStr = extractTextFromChildren(children).replace(/^#+\s*/, '');
            const id = `heading-${slugify(textStr)}`;
            return <h1 id={id} className="md-heading md-h1">{cleanHeadingChildren(children)}</h1>;
          },
          h2({ children }) {
            const textStr = extractTextFromChildren(children).replace(/^#+\s*/, '');
            const id = `heading-${slugify(textStr)}`;
            return <h2 id={id} className="md-heading md-h2">{cleanHeadingChildren(children)}</h2>;
          },
          h3({ children }) {
            const textStr = extractTextFromChildren(children).replace(/^#+\s*/, '');
            const id = `heading-${slugify(textStr)}`;
            return <h3 id={id} className="md-heading md-h3">{cleanHeadingChildren(children)}</h3>;
          },
          p({ children }) {
          ...
          code({ inline, className, children, ...props }: any) {
            ...
            if (!inline && lang === 'mermaid') {
              return <MermaidDiagram chart={codeString} onExpand={handleExpand} />;
            }
      ...
      {/* Internal Full-Screen Mermaid Modal */}
      {internalExpandedSvg && (
        <div
          ...
          <div className="diagram-modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            >
              <div
                ...
                dangerouslySetInnerHTML={{ __html: internalExpandedSvg }}
              />
            </div>
          </div>
        </div>
